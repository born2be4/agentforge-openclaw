...
### Шаг 1b. Выбери профиль сложности (lite / standard / full)

**Lite (3-5 файлов)** — для масок и простых ролей:
- `AGENTS.md`, `SOUL.md`, `USER.md` (+ опционально `TOOLS.md`, `IDENTITY.md`)
- memorySearch: обычно `false`
- Без автодневников и handoff

**Standard (8-10 файлов)** — для регулярной операционки:
- Lite + `MEMORY.md` + `TOOLS.md` + `memory/lessons.md` + `memory/projects-log.md`
- memorySearch: по задаче

**Full (12-13 файлов)** — для долгих контекстных ролей:
- Вся структура + `memory/patterns.md`, `memory/architecture.md`, `BOOTSTRAP.md`, `memory/handoff.md`
- memorySearch: чаще `true`
- Можно добавлять auto handoff/diary

Правило: **не перегружай лёгкие роли full-архитектурой без необходимости**.
...
- `memorySearch`: включать осознанно (см. шаг 2a), не по умолчанию для всех
- `tools.deny`: `gateway` ВСЕГДА; `cron`, `exec` по ситуации
- `heartbeat.every`: "0" если не нужен мониторинг

### Шаг 2a. Decision rules для memorySearch

Включай `memorySearch.enabled: true`, если одновременно выполняются 2+ условия:
- агент ведёт длинные диалоги и должен помнить контекст между сессиями;
- регулярные вопросы «что мы решали раньше?»;
- решения завязаны на историю договорённостей/исключений.

Оставляй `false`, если:
- роль узкая и одношаговая (copywriter-mask, simple QA);
- высокая стоимость памяти не окупается;
- критична максимальная изоляция и минимальный контекст.
...
⚠️ Safe-restart policy: рабочий агент НЕ должен перезапускать gateway сам. Перезапуск только внешним оператором/координатором (терминал, infra-агент, CI job).
